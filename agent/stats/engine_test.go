	...
	tcs := []struct {
	...
	}{
		{
			...
			setCSIClientExpectation: func(csi *mock_csiclient.MockCSIClient) {
				csi.EXPECT().GetVolumeMetrics(gomock.Any(), "vol-12345", fmt.Sprintf("%s/taskarn_vol-12345", taskresourcevolume.EBSSourcePrefix)).Return(&csiclient.Metrics{
		...
		},
		{
			name: "Failure",
			setCSIClientExpectation: func(csi *mock_csiclient.MockCSIClient) {
				csi.EXPECT().GetVolumeMetrics(gomock.Any(), "vol-12345", fmt.Sprintf("%s/taskarn_vol-12345", taskresourcevolume.EBSSourcePrefix)).Return(nil, errors.New("err")).Times(1)
			},
			expectedMetrics: nil,
			numMetrics:      0,
		},
		{
			name: "TimeoutFailure",
			setCSIClientExpectation: func(csi *mock_csiclient.MockCSIClient) {
				csi.EXPECT().GetVolumeMetrics(gomock.Any(), "vol-12345", fmt.Sprintf("%s/taskarn_vol-12345", taskresourcevolume.EBSSourcePrefix)).Return(nil, errors.New("rpc error: code = DeadlineExceeded desc = context deadline exceeded")).Times(1)
			},
			expectedMetrics: nil,
			numMetrics:      0,
		},
